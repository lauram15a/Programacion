...
public class ExcepcionJuego {
     ...
     */
    public static String[] pedirInstruccion() {
        ...
        while (comprobarInstruccion(listaLineaComandos) == false) {
            ...
            listaLineaComandos = pedirInstruccion();
        }
        ...
        /* No es necesario porque yo doy las dimensiones del tablero
        *
        //primera instrucción que se meta sea la de determinar las dimensiones del tablero y seleccionar la dificultad
        if (contadorTurno == 0) {
            while (!"NUEVA".equals(listaLineaComandos[0].toUpperCase()) && !"S".equals(listaLineaComandos[0].toUpperCase())) {
            ...
            while (tamanioTableroFilas(numFilas) != true || tamanioTableroColumnas(numColumnas) != true) {
                listaLineaComandos = pedirInstruccion(contadorTurno);
    ...
    public static void imprimirLineaComandos() {
        System.out.println("\nNueva <filas> <columnas> <Dificultad>: Nueva partida (Dificultad: BAJA, MEDIA, ALTA, IMPOSIBLE).");
        System.out.println("G <fila> <columna>: colocar girasol.");
        System.out.println("L <fila> <columna>: colocar LanzaGuisantes.");
        System.out.println("N <fila> <columna>: colocar Nuez.");
        System.out.println("P <fila> <columna>: colocar PetaCereza.");
    ...
    public static boolean comprobarInstruccion(String[] listaLineaComandos) {
        ...
        } if ("PASO".equals(listaLineaComandos[0].toUpperCase())) {  //ayuda
            if (listaLineaComandos.length != 1) {  //mas de un elemento en lista
                return false;
            } else {   //mas de un elemento en lista
                return true;
            }
        } else if ("NUEVA".equals(listaLineaComandos[0].toUpperCase())) {  //nueva partida
        ...
        } else if ("L".equals(listaLineaComandos[0].toUpperCase()) || "G".equals(listaLineaComandos[0].toUpperCase()) || "N".equals(listaLineaComandos[0].toUpperCase()) || "P".equals(listaLineaComandos[0].toUpperCase())) {  //plantas
    ...
    public static boolean finPartida(Objeto[][] tablero, int numFilas, int numColumnas, int contadorZombis, int numZombis, int contadorTurno) {
        if (finPartidaZmbColumna0(tablero, numFilas, numColumnas) == true){ //zmbs en columna 0
            return true;
